...
type RateLimitIdentity = "ip" | "user" | "eventStaff" | "email";
...
export const rateLimitPolicies = {
  ...
  checkInSync: { name: "check-in-sync", limit: 300, windowMs: oneMinute, identity: "eventStaff" },
...
const policyKey = (policy: RateLimitPolicy, req: Request): string => {
  ...
  if (policy.identity === "eventStaff") return `${req.params.eventId}:${req.jwtPayload.id}`;
