...
describe("Platform rate-limit policy", () => {
  ...
  it("isolates sync limits by event and staff member", async () => {
    const app = express();
    app.use((req: Request, _res, next) => {
      req.jwtPayload = { id: req.header("x-test-user") || "staff-1", email: "staff@example.com", role: ERole.STAFF };
      next();
    });
    app.post(
      "/events/:eventId/sync",
      throttleMiddleware({ ...rateLimitPolicies.checkInSync, limit: 1 }, new MemoryStore()),
      (_req, res) => res.status(204).end(),
    );
    expect((await request(app).post("/events/a/sync")).status).to.equal(204);
    expect((await request(app).post("/events/a/sync")).status).to.equal(429);
    expect((await request(app).post("/events/b/sync")).status).to.equal(204);
    expect((await request(app).post("/events/a/sync").set("x-test-user", "staff-2")).status).to.equal(204);
  });
});
